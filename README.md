...
- **v1 scope**: JSON OpenAPI specs (convert YAML first), GraphQL endpoints, and
  SOAP/WSDL services; bearer / no auth. `search` is keyword-based over the
  operation index - no embeddings or freeform-doc search. Each `execute` is a
  fresh subprocess (no persistent state between calls).
